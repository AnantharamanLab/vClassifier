...
awk 'BEGIN {n=0;} /^>/ {if(n%100==0){file=sprintf("chunk%d.fa",n);} print >> file; n++; next;} { print >> file; }' < queries.num
...
for i in *hmm
do
    echo "hmmsearch -o /dev/null --domtblout "$i".hmmout -E 1e-3 --cpu 1 $VOG_database/$i $wd/gene_calling/queries.protein.faa"
...
count=`ls -1 *monophyletic_groups_with_seqID_for_subfamily_assignment_output 2>/dev/null | wc -l`
if [ $count != 0 ]
then
    cat *monophyletic_groups_with_seqID_for_subfamily_assignment_output|grep 'query' > $wd/query_subfamily_assignment_output
else
    touch $wd/query_subfamily_assignment_output
fi
